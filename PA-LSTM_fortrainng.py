...
attrs_path = 'D:\\MyGIS\\hydro_dl_project\\camels_data\\datafor_rm\\attributedata.csv'
...
def create_model(input_xd_shape, input_xs_shape, seed):
    ...
    xd_hydro_scale = ScaleLayer(name='Scale')(xd_input_forprnn)
    print("xd_hydro_scale",xd_hydro_scale)

    ealstm_hn, ealstm_cn = LSTM_layer(input_xd = 5, input_xs = 27, hidden_size=128, seed=seed)([xd_hydro_scale,xs_input])
...
for i in range(len(basin_climate1_id)):
    ...
    save_palstm = f'{working_path}/results/California_PA-LSTM_Models/PA-LSTM_{basin_climate1_id[i]}.h5'

    model = create_model(input_xd_shape=(train_x.shape[1], train_x.shape[2]), input_xs_shape=(local_train.shape[1]),
                         seed=200)
    model.summary()

    train_xs_nor = normalize_minmax_xs(local_train)
    train_y_nor, train_y_mean, train_y_std = normalize_y(train_y)

    palstm_history = train_model(model=model, train_xd=train_x,
                                      train_xs=train_xs_nor, train_y=train_y, ep_number=200, lrate=0.01,
