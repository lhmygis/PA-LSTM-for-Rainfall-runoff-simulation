...
from libs_tf.Class_PA-LSTM import PRNNLayer, ConvLayer, ScaleLayer, LSTMLayer, Regional_EALSTM
...
def normalize_T(data):
    test_mean = np.mean(data, axis=0, keepdims=True)
    test_std = np.std(data, axis=0, keepdims=True)
    test_scaled = (data - test_mean) / test_std
    return test_scaled

def find_pair_with_difference_t_warming(arr):
    n = len(arr)
    for i in range(n):
...
def generate_train_test(train_set, test_set1, test_set2, wrap_length, attrs_file, basin_id):
    ...
    num1, num2 = find_pair_with_difference_t_warming(test_set1.values[:, 1])
    print("num1:", num1)
    print("num2", num2)

    test_set_true = test_set1.values
    print("test_set_true.type:",type(test_set_true))
    print("test_set_true.shape:", test_set_true.shape)
    print("test_set_true.T:", test_set_true[:,1])
    t_warming_true = abs(test_set_true[num1, 1] - test_set_true[num2, 1])
    print("t_warming_true:", t_warming_true)
    test_set_true[:, 1] = test_set_true[:, 1] + t_warming_true
    test_x_np_t = test_set_true[:, :-1]
    test_y_np_t = test_set_true[:, -1:]
    print("test_set_true[:, 1]_warming:", test_set_true[:, 1])




    test_set_nor = test_set2.values
    print("test_set_nor.type:",type(test_set_nor))
    print("test_set_nor.shape:", test_set_nor.shape)
    print("test_set_nor.T:", test_set_nor[:,1])
    test_set_nor[:,1] = normalize_T(test_set_nor[:,1])
    print("test_set.normalize_T:", test_set_nor[:,1])
    t_warming_nor = abs(test_set_nor[num1,1] - test_set_nor[num2, 1])
    print("t_warming_nor:", t_warming_nor)

    test_set_nor[:, 1] = test_set_nor[:, 1] + t_warming_nor

    train_x_np = train_set.values[:, :-1]
    train_y_np = train_set.values[:, -1:]
    test_x_np_n = test_set_nor[:, :-1]
    test_y_np_n = test_set_nor[:, -1:]
    ...
    test_x_t = np.expand_dims(test_x_np_t, axis=0)
    test_y_t = np.expand_dims(test_y_np_t, axis=0)

    test_x_n = np.expand_dims(test_x_np_n, axis=0)
    test_y_n = np.expand_dims(test_y_np_n, axis=0)

    for i in range(wrap_number_train):
        train_x[i, :, :] = train_x_np[i * 365:(wrap_length + i * 365), :]
        train_y[i, :, :] = train_y_np[i * 365:(wrap_length + i * 365), :]

    return train_x, train_y, local_static_x_for_train, test_x_t, test_y_t, test_x_n, test_y_n, local_static_x_for_test

def create_model(input_xt_shape, input_xn_shape, input_xs_shape, seed):
    xt_input_forprnn = Input(shape=input_xt_shape, batch_size=1, name='Input_xt')
    xn_input_forealstm = Input(shape=input_xn_shape, batch_size=1, name='Input_xn')
    xs_input = Input(shape=input_xs_shape, batch_size=1, name='Input_xs')

    hydro_output = PRNNLayer(mode='normal', name='Hydro')(xt_input_forprnn)
    print("hydro_output",hydro_output)


    xd_hydro = Concatenate(axis=-1, name='Concat')([xn_input_forealstm, hydro_output])
    print("xd_hydro",xd_hydro)

    xd_hydro_scale = ScaleLayer(name='Scale')(xd_hydro)
    print("xd_hydro_scale",xd_hydro_scale)

    ealstm_hn, ealstm_cn = Regional_EALSTM(input_xd = 9, input_xs = 27, hidden_size=128, seed=seed)([xd_hydro_scale,xs_input])
    fc2_out = Dense(units=1)(ealstm_hn)

    fc2_out = K.permute_dimensions(fc2_out, pattern=(1,0,2))  # for test model

    model = Model(inputs = [xt_input_forprnn, xn_input_forealstm, xs_input], outputs = fc2_out)
    return model
...
def test_model(model, test_xt, test_xn, test_xs, save_path):

    model.load_weights(save_path, by_name=True)
    pred_y = model.predict(x = [test_xt,test_xn, test_xs], batch_size=1)
    return pred_y
...
for i in range(len(basin_climate1_id)):
    ...
    test_set2 = hydrodata[hydrodata.index.isin(pd.date_range(testing_start, testing_end))]

    print(f"The training data set is from {training_start} to {training_end}, with a shape of {train_set.shape}")
    print(f"The testing data set is from {testing_start} to {testing_end}, with a shape of {test_set1.shape}")
    print(f"The testing data set is from {testing_start} to {testing_end}, with a shape of {test_set2.shape}")

    wrap_length = 2190  # 6-years for a train
    train_x, train_y, local_train, test_xt, test_yt,test_xn, test_yn, local_test = generate_train_test(train_set, test_set1, test_set2,
    ...
    print(f'{train_x.shape}, {train_y.shape},{local_train.shape}, {test_xt.shape},{test_yt.shape}, {test_xn.shape},{test_yn.shape}and {local_test.shape}')

    Path(f'{working_path}/results').mkdir(parents=True, exist_ok=True)
    save_path_palstm = f'{working_path}/results/California_PA-LSTM_Models/pa_lstm_{basin_climate1_id[i]}.h5'

    model = create_model(input_xt_shape=(test_xt.shape[1], test_xt.shape[2]), input_xn_shape=(test_xn.shape[1], test_xn.shape[2]),
                         input_xs_shape = (local_train.shape[1]) , seed=200)

    flow = test_model(model=model, test_xt = test_xt, test_xn = test_xn,test_xs = local_test,
                      save_path=save_path_palstm)

    nse_test = nse_metrics(test_yn, flow)
